	...
	<table width="100%" border="0" align="center">
		...
		<c:forEach var="goods" items="${goodsList}" varStatus="status">
			<tr
				...
				<td class="col1"><a
					href="detail?id=${goods.id}">${goods.description}</a></td>
				<td class="col2">${goods.price}</td>
				<td class="col3"><a
					href="add?pagename=list&id=${goods.id}&name=${goods.name}&price=${goods.price}">添加到購物車</a></td>
			</tr>
		</c:forEach>
	</table>
	<div align="center">
		<ul class="pagination">
			<li><a href="paging?page=prev">«</a></li>
			<c:forEach var="page" begin="1" end="${totalPageNumber}">
				<li><a
					<c:if test="${page == currentPage }">
						class="active"
					</c:if>
					href="paging?page=${page}">${page}</a></li>
			</c:forEach>
			<li><a href="paging?page=next">»</a></li>
		</ul>
	</div>
